...
public class ModalPhotoUploadAction extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("-----------------------------");
		System.out.println("game_idx=" + game_idx);
		System.out.println("-----------------------------");
		
		
		//game_idx에 저장된 이전 파일 삭제
		Game1Vo vo = Game1Dao.getInstance().selectOne(game_idx);
		
		System.out.println("-----------------------------");
		System.out.println(vo.getImg_filename());
		System.out.println("-----------------------------");
		
		if(vo.getImg_filename()!=null) {
		...
		//{"p_filename":"%s"}
		String json = String.format("{\"img_filename\":\"%s\"}", img_filename);
